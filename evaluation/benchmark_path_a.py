...
# ========================================================
# ENVIRONMENT & PATH SETUP (Aligned with Test 7 & Baseline)
# ========================================================
...
VARIANT_REGISTRY = {
    "b1": {
        "cfg": CFG_MobileMamba_B1,
        "res": 256,
        "resize": 292,
        "default_ckpt": "/content/MobileMamba/weights/MobileMamba_B1/mobilemamba_b1.pth",
    },
    "b2": {
        "cfg": CFG_MobileMamba_B2,
        "res": 384,
        "resize": 438,
        "default_ckpt": "/content/MobileMamba/weights/MobileMamba_B2/mobilemamba_b2.pth",
    },
    "b4": {
        "cfg": CFG_MobileMamba_B4,
        "res": 512,
        "resize": 585,
        "default_ckpt": "/content/MobileMamba/weights/MobileMamba_B4/mobilemamba_b4.pth",
    },
}
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Proper MobileMamba ImageNet Path A ([1, 3, 2]) Benchmark"
    )
    parser.add_argument(
        "--variant",
        type=str,
        default="b1",
        choices=["b1", "b2", "b4", "B1", "B2", "B4"],
        help="Model variant: b1, b2, or b4",
    )
    ...
    parser.add_argument(
        "--amp",
        action="store_true",
        help="Enable AMP FP16. (Omit to run pure FP32 matching Test 7)",
    )
...
def load_model(variant_key, checkpoint_path, forward_type, device):
    ...
    # 1. Base model instantiation with the selective scan kernel
    model = MobileMamba(
        **cfg,
        num_classes=1000,
        distillation=False,
    ...
    # 2. Path A Modification: Bypass Stage 1 Block-1 -> Topology [1, 3, 2]
    log("Applying Path A Pruning: blocks1[1] = nn.Identity() (Stage 1 Depth: 2 -> 1)")
...
def main():
    ...
    if args.save_path is None:
        args.save_path = f"/content/{variant.upper()}_ImageNet_PathA_PROPER.json"
    ...
    # 2 & 3. MODEL
    # ========================================================
    log(f"Instantiating MobileMamba_{variant.upper()}_PathA (forward_type={args.forward_type})...")
    ...
    log("Model successfully loaded and modified for Path A.")
    ...
    # ========================================================
    # 4.5 ISOLATED FORWARD LATENCY (Aligned with Test 7 & Baseline)
    # ========================================================
    ...
    # ========================================================
    forward_fps = (
        args.batch_size / (isolated_stats["mean_ms"] / 1000.0)
        if isolated_stats["mean_ms"] > 0
        else 0.0
    )
    e2e_fps = (
        args.batch_size / (e2e_stats["mean_ms"] / 1000.0)
        if e2e_stats["mean_ms"] > 0
        else 0.0
    )
    ...
    report = {
        "experiment": {
            "name": f"MobileMamba_{variant.upper()}_PathA_PROPER",
            "model": f"MobileMamba_{variant.upper()}_[1,3,2]",
            "topology": "[1, 3, 2] (Stage 1 Block-1 bypassed with nn.Identity)",
    ...
    print()
    print("=" * 70, flush=True)
    print(f"{variant.upper()} PATH A ([1, 3, 2]) + ImageNet-1K COMPLETE", flush=True)
    print("=" * 70, flush=True)
